...
# port-open check is not admission validation. In one fanout a substantial
# minority of probes never established a connection after their port checks
# passed, and recording those as 0 Mbps would have implicated healthy clients as
# failing. So a known-bad endpoint is refused, and its record explicitly says it
# is not a zero-throughput result.
...
check_contains "the refusal explains why it matters" "implicated healthy clients as failing" \
    "$BIN" endpoints --check speedtest.xmission.com:5202
