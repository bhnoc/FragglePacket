...
pub fn render_https_panel(frame: &mut Frame, area: Rect, app: &App) {
    ...
    for (target, result) in &state.https_results {
        ...
        let diagnosis_style = match &result.diagnosis {
            ...
            HttpsDiagnosis::TlsTimeout | HttpsDiagnosis::MtuBlackhole => Style::default().fg(TERM_RED).add_modifier(Modifier::BOLD),
        ...
        rows.push(Row::new(vec![
            ...
            Cell::from(format!("{}ms", result.total_time_ms)).style(Style::default().fg(TERM_GREEN_DIM)),
    ...
    .header(
        Row::new(vec!["Target", "DNS", "TCP", "TLS", "HTTP", "Total", "Diagnosis"])
    ...
    if state.diagnoses.is_empty() {
        diagnosis_lines.push(Line::from(
            Span::styled("No issues detected", Style::default().fg(TERM_GREEN))
        ));
    } else {
        for (i, diag) in state.diagnoses.iter().take(3).enumerate() {
            ...
            diagnosis_lines.push(Line::from(vec![
                ...
                Span::styled(format!("{:?}: ", diag.issue), Style::default().fg(severity_color).add_modifier(Modifier::BOLD)),
                Span::styled(&diag.description[..diag.description.len().min(80)], Style::default().fg(TERM_GREEN_DIM)),
            ]));
        }
    }
