...
fn render_category_buttons(f: &mut Frame, app: &App, area: Rect) {
    let mut button_text = vec![
        Line::from(vec![
            Span::styled("  Test Categories  ", Style::default().fg(TERM_CYAN).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(""),
    ];
        ...
        if let Some(results) = results {
            for result in results {
                ...
                if cat_matches {
                    ...
                    items.push(ListItem::new(Line::from(vec![
                        Span::styled(format!(" {} ", result.name), Style::default().fg(TERM_GREEN).add_modifier(Modifier::BOLD)),
                    ...
                    for (key, value) in result.metrics.iter().take(3) {
                        items.push(ListItem::new(Line::from(vec![
                            Span::raw("   "),
                            Span::styled(format!("{}: {:.1}", key, value), Style::default().fg(TERM_GREEN_DIM)),
                        ])));
                    }
                    ...
                    for diag in &result.diagnoses {
                        items.push(ListItem::new(Line::from(vec![
                            Span::raw("   "),
                            Span::styled(format!("[{:?}] {}", diag.severity, diag.title), Style::default().fg(TERM_AMBER)),
                        ])));
                    }
            ...
            items.push(ListItem::new(Line::from(vec![
                Span::styled("  No results yet. Press ENTER to run test.", Style::default().fg(TERM_GREEN_DIM)),
            ])));
        }
    } else {
        // Show overview of all categories
        items.push(ListItem::new(Line::from(vec![
            Span::styled("All Test Categories", Style::default().fg(TERM_CYAN).add_modifier(Modifier::BOLD)),
        ])));
        items.push(ListItem::new(Line::from("")));
        
        if let Some(results) = results {
            for (name, _) in CATEGORIES {
                let count = results.iter().filter(|r| {
                    format!("{:?}", r.category).contains(name.replace(" ", "").as_str())
                }).count();
                
                items.push(ListItem::new(Line::from(vec![
                    Span::styled(format!(" {} ", name), Style::default().fg(TERM_GREEN)),
                    Span::styled(format!("{} test(s)", count), Style::default().fg(TERM_GREEN_DIM)),
                ])));
            }
        } else {
            items.push(ListItem::new(Line::from(vec![
                Span::styled("  No tests run yet. Select category (1-10) and press ENTER.", Style::default().fg(TERM_GREEN_DIM)),
            ])));
        }
    }
...
fn render_status_bar(f: &mut Frame, app: &App, area: Rect) {
    let status_text = if let Some(cat) = app.selected_category {
        format!(" Category: {} | ENTER=Run | ESC=Back | TAB=Switch View ", CATEGORIES[cat].0)
    } else {
        " Select Category (1-10) | ENTER=Run All | ESC=Back | TAB=Switch View ".to_string()
    };
    
    let paragraph = Paragraph::new(status_text)
        .style(Style::default().bg(TERM_GREEN_DARK).fg(TERM_GREEN).add_modifier(Modifier::BOLD))
