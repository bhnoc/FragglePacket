...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Block, BorderType, Borders, Cell, Clear, Gauge, List, ListItem, ListState, Paragraph, Row, Sparkline, Table, TableState, Wrap,
    },
    Frame, Terminal,
};
use rayon::prelude::*;
...
use fraggle_packet::network_tests::{test_https_stages, HttpsTestResult};
...
use fraggle_packet::framework::result::{
    TestResult as FrameworkTestResult, TestStatus as FrameworkTestStatus,
};
...
pub enum TestUpdate {
    ...
    TracepathOutput {
        line: String,
    }, // Stream tracepath output line-by-line
}
...
impl App {
    ...
    pub fn run_tracepath(&mut self, index: usize) {
        if self.tracepath_running {
            self.log_messages
                .push("[TRACEPATH] Already running".to_string());
        ...
        let target = {
            ...
            if let Some(r) = s.results.get(index) {
                ...
                self.log_messages
                    .push("[TRACEPATH] Invalid index".to_string());
                return;
            }
        };
        ...
        let tx = match &self.test_tx {
            ...
            None => {
                self.log_messages
                    .push("[TRACEPATH] No channel available".to_string());
                return;
            }
        };
        ...
        self.log_messages
            .push(format!("[TRACEPATH] Starting for {}", target));
        self.show_popup = true;
    ...
    pub fn start_testing(
        ...
        thread::spawn(move || {
            ...
            let targets_vec: Vec<(usize, String, String, u16)> = targets
                .iter()
                .enumerate()
            ...
            for (index, target, desc, port) in targets_vec {
                ...
                let _ = tx_clone.send(TestUpdate::Started {
                    target: target.clone(),
                });
                ...
                let handle = thread::spawn(move || {
                    ...
                    let result = test_single_target(
                        &target, &desc, port, min_mtu, max_mtu, timeout_ms, retries,
                    );
                    ...
                    *count += 1;
                    ...
                    // Send result
                    let _ = tx_clone.send(TestUpdate::Complete { index, result });
                });
    ...
    pub fn run_https_test(&mut self, target: &str) {
        ...
        self.log_messages
            .push(format!("Running HTTPS test on {}...", target));
        self.show_popup = true;
        ...
        thread::spawn(move || {
            ...
            state
                .https_results
                .insert(target_str.clone(), result.clone());
    ...
    pub fn run_category(&mut self, category: TestCategory) {
        ...
        self.log_messages
            .push(format!("Running {:?} test on {}...", category, target));
        self.show_popup = true;
        ...
        self.popup_message = format!(
            "Completed {:?} tests on {}\n{}",
            category,
            target,
            self.log_messages.last().unwrap_or(&"Done".to_string())
        );
    }
    ...
    pub fn run_all_tests_on_current_target(&mut self) {
        ...
        self.log_messages
            .push(format!("Running ALL tests for {}", target));
    ...
    pub fn run_category_on_all_targets(&mut self, category: TestCategory) {
        ...
        for target in &targets {
            ...
            for result in results {
                ...
                self.log_messages
                    .push(format!("{} {} on {}", status_str, result.name, target));
            }
        }
    ...
    pub fn retest_target(
        ...
        self.log_messages
            .push(format!("[RETEST] Starting retest for index {}", index));
        ...
        let (target, desc) = {
            ...
            if let Some(r) = s.results.get(index) {
                ...
                self.log_messages
                    .push(format!("[RETEST] ERROR: Index {} not found", index));
                return;
            }
        };

        self.log_messages
            .push(format!("[RETEST] Target: {}, Desc: {}", target, desc));
        ...
        // Mark as testing
        {
            ...
            if let Some(r) = s.results.get_mut(index) {
                ...
                self.log_messages
                    .push(format!("[RETEST] Status set to Testing"));
            }
        }

        let tx_clone = new_tx.clone();
        self.log_messages
            .push(format!("[RETEST] Spawning test thread..."));
        ...
        thread::spawn(move || {
            ...
            let result =
                test_single_target(&target, &desc, port, min_mtu, max_mtu, timeout_ms, retries);

            let _ = tx_clone.send(TestUpdate::Complete { index, result });
        });

        self.log_messages
            .push(format!("[RETEST] Thread spawned, waiting for results..."));
    }
            ...
            while let Ok(update) = rx.try_recv() {
                ...
                match update {
                    ...
                    TestUpdate::Complete { index, result } => {
                        self.log_messages.push(format!(
                            "[UPDATE] Complete for index {}: icmp={:?} tcp={:?} udp={:?} quic={:?}",
                        ...
                        if let Some(r) = state.results.get_mut(index) {
                            ...
                            if has_results {
                                ...
                                self.log_messages.push(format!(
                                    "[COMPLETE] Target {} - Press ESC to close",
                                    r.desc
                                ));

                                // Update MTU history for sparkline
                                if let Some(mtu) =
                                    result.icmp_mtu.or(result.tcp_mtu).or(result.udp_mtu)
                                {
                                ...
                                self.log_messages.push(format!(
                                    "[FAILED] Target {} - Press ESC to close",
                                    r.desc
                                ));
                    ...
                    TestUpdate::TracepathComplete { index, hop_count } => {
                        self.log_messages
                            .push(format!("[TRACEPATH] Complete: {} hops parsed", hop_count));
                        self.tracepath_running = false;

                        if let Some(r) = state.results.get(index) {
                            self.tracepath_output.push(String::new());
                            self.tracepath_output
                                .push(format!("=== Tracepath complete for {} ===", r.desc));
                            self.tracepath_output
                                .push(format!("Found {} hops", hop_count));
                            self.tracepath_output.push(String::new());
    ...
    pub fn run_selected_fuzzer(&mut self) {
        ...
        self.log_messages
            .push(format!("Starting {} fuzzer on {}", mode_str, target));
        ...
        thread::spawn(move || {
            use fraggle_packet::fuzzing::{run_campaign, FuzzMode, PacketContext};
            ...
            match run_campaign(&ctx, mode, &output) {
                Ok(fuzz_result) => {
                    let mut state = state_clone.lock().unwrap();
                    state.fuzzing_results.insert(
                        mode_str_owned.clone(),
                        FuzzingResult {
                            mode: mode_str_owned,
...
fn render_target_detail(frame: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(target) = state.results.get(app.selected_target) {
        ...
        details.push(Line::from(vec![
            ...
            Span::styled(
                " to run tracepath (opens in popup)",
                Style::default().fg(TERM_GREEN_DIM),
            ),
        ]));
        ...
        let result_msg = if would_work {
            ...
            vec![
                ...
                Line::from(vec![
                    Span::raw("  "),
                    Span::styled(
                        format!("Interface MTU ({}) > Path MTU ({})", simulated, min_display),
                ...
                Line::from(vec![
                    Span::raw("  "),
                    Span::styled(
                        "Packets would be fragmented/dropped!",
        ...
        whatif.extend(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    (simulated.saturating_sub(40)).to_string(),
                    Style::default().fg(TERM_GREEN),
                ),
            ]),
            Line::from(""),
            ...
            Line::from(vec![
                Span::styled(" WireGuard:     ", Style::default().fg(TERM_GREEN_DIM)),
                Span::styled(
                    (simulated.saturating_sub(60)).to_string(),
                    Style::default().fg(TERM_GREEN),
                ),
            ]),
            Line::from(vec![
                ...
                Span::styled(
                    (simulated.saturating_sub(100)).to_string(),
                    Style::default().fg(TERM_GREEN),
                ),
            ]),
        ]);

        let whatif_widget = Paragraph::new(whatif).block(
            Block::default()
...
fn render_simulator(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Row> = state
        ...
        .map(|r| {
            ...
            Row::new(vec![
                Cell::from(r.desc.chars().take(20).collect::<String>())
                    .style(Style::default().fg(TERM_GREEN)),
...
fn render_help(frame: &mut Frame, area: Rect) {
    let help_text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Test Framework (10 test categories)",
                Style::default().fg(TERM_GREEN),
            ),
        ]),
        Line::from(""),
        Line::from(Span::styled(
            "TEST FRAMEWORK (in Test Panel [T])",
            Style::default().fg(TERM_AMBER),
        )),
        Line::from(vec![
            Span::styled("  1-0    ", Style::default().fg(TERM_CYAN)),
            Span::styled(
                "Select test: 1=DNS 2=MTU 3=HTTPS 4=TCP 5=RTT",
                Style::default().fg(TERM_GREEN),
            ),
        ]),
        Line::from(vec![
            Span::styled("         ", Style::default().fg(TERM_CYAN)),
            Span::styled(
                "6=Loss 7=Path 8=IPv6 9=App 0=Fuzz",
                Style::default().fg(TERM_GREEN),
            ),
        ]),
        Line::from(vec![
            Span::styled("  Enter  ", Style::default().fg(TERM_CYAN)),
            Span::styled(
                "Run selected test (smart: single/all targets)",
                Style::default().fg(TERM_GREEN),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                "Run ALL 10 tests on current target",
                Style::default().fg(TERM_GREEN),
            ),
        ]),
        Line::from(""),
...
fn render_footer(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let help_hint = match app.mode {
        ...
        _ => {
            " [?]Help [1]Dash [F]Fuzz [H]HTTPS [T]Tests [3]Sim [t]Trace [r]Retest [s]Save [q]Quit "
        }
    };
        ...
        if let Event::Key(key) = event::read()? {
            ...
            match key.code {
                ...
                KeyCode::Esc => {
                    ...
                    match app.mode {
                        AppMode::FuzzingPanel | AppMode::HttpsPanel => {
                            app.mode = AppMode::Dashboard
                        }
...
pub fn create_demo_state() -> AppState {
    ...
    let hops = vec![
        ...
        HopInfo {
            hop: 3,
            addr: "72.14.215.85".into(),
            mtu: Some(1500),
            rtt_ms: Some(5.1),
        },
        HopInfo {
            hop: 4,
            addr: "142.250.169.174".into(),
            mtu: Some(1500),
            rtt_ms: Some(8.2),
        },
    ];
    ...
    AppState {
        ...
        mtu_history: vec![
            1500, 1500, 1500, 1480, 1500, 1500, 1420, 1500, 1500, 1500, 1500, 1460, 1500, 1500,
            1500, 1400, 1500, 1500, 1500, 1500,
        ],
        simulated_mtu: 1500,
        fuzzing_results: std::collections::HashMap::new(),
        https_results: std::collections::HashMap::new(),
