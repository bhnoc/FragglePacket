...
impl NetworkTest for ApplicationTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        if http11 { protocols_supported += 1; }
        if http2_result.unwrap_or(false) { protocols_supported += 1; }
        if http3_result.unwrap_or(false) { protocols_supported += 1; }
        if ws_result.unwrap_or(false) { protocols_supported += 1; }
        ...
        if protocols_supported == 0 {
            ...
            result.set_status(TestStatus::Success);
            result.add_metadata("verdict", format!("{} protocols supported", protocols_supported));
        }
...
fn check_http2_support(target: &str, timeout_secs: u64) -> Result<bool, Box<dyn std::error::Error>> {
            ...
            if stream.write_all(request.as_bytes()).is_ok() {
                ...
                if let Ok(n) = stream.read(&mut buf) {
                    ...
                    if response.to_lowercase().contains("alt-svc") {
                        ...
                        if lower.contains("h3=") || lower.contains("h3-29=") || lower.contains("h3-27=") {
                            return Ok(true);
                        }
                    }
                }
...
fn check_websocket_connectivity(target: &str, timeout_secs: u64) -> Result<bool, Box<dyn std::error::Error>> {
    let addr = format!("{}:80", target);
