...
impl NetworkTest for PathAnalysisTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
            ...
            Err(_) => {
                ...
                match Command::new("tracepath")
                    ...
                    Err(e) => {
                        ...
                        result.add_metadata("error", format!("traceroute/tracepath not available: {}", e));
                        return Ok(result);
                    }
                }
            }
        ...
        for (i, hop) in hops.iter().enumerate() {
            ...
            if hop.timeout {
                ...
                let latency_str = if let Some(data) = latency_data {
                    ...
                    format!("{} (avg {:.1}ms, loss {:.0}%)", hop.addr, data.avg, data.loss_percent)
                } else {
        ...
        result.add_metadata("final_hop", hops.last().map(|h| h.addr.clone()).unwrap_or_default());
        
        // TTL analysis
        let final_hop_count = hops.len();
        let estimated_ttl_used = final_hop_count;
        
        result.add_metric("hops_to_target", final_hop_count as f64);
        result.add_metric("estimated_initial_ttl", estimate_initial_ttl(final_hop_count) as f64);
        ...
        if final_hop_count >= (self.max_hops as usize) && hops.last().map(|h| h.timeout).unwrap_or(false) {
            result.add_diagnosis(Diagnosis::new(
                DiagnosisSeverity::Error,
                "Target Not Reached".to_string(),
                format!("Reached max hops ({}) without finding target", self.max_hops),
            ...
            if let Some((hop_num, increase)) = hop_latency_increase.first() {
                if *increase > 50.0 {
                    result.add_diagnosis(Diagnosis::new(
                        ...
                        format!("Hop {} adds {:.1}ms latency (largest contributor)", hop_num, increase),
                    ).with_recommendation(format!("Investigate hop {} for congestion or routing issues", hop_num)));
                }
            }
        }
        
        // Calculate average RTT from all non-timeout hops
        let rtts: Vec<f64> = hops.iter().filter(|h| !h.timeout).map(|h| h.rtt_ms).collect();
        if !rtts.is_empty() {
        ...
        if has_timeouts {
            ...
            result.add_diagnosis(Diagnosis::new(
                ...
                "Some hops in path did not respond (may be normal for filtered routers)".to_string(),
            ...
            result.add_diagnosis(Diagnosis::new(
                ...
                format!("Path has {} hops with high latency", 
        ...
        let unique_addrs: std::collections::HashSet<_> = hops.iter()
            .filter(|h| !h.timeout)
            .map(|h| &h.addr)
            .collect();
