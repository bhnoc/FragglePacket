...
impl NetworkTest for TcpSegmentationTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        result.add_metadata(
            "cli_command",
            format!(
                "curl -so /dev/null --write-out '%{{size_download}}' https://{}",
                target
            ),
        );
        result.add_metadata(
            "cli_tcpdump",
            format!(
                "tcpdump -i any -c 20 'tcp and host {}' -v | grep -E 'length|mss'",
                target
            ),
        );
        result.add_metadata(
            "cli_note",
            "Tests TCP with segment sizes: 100, 536, 1024, 1460 bytes",
        );
        ...
        if successful_sizes.is_empty() {
                ...
                if max_successful < 536 {
                    ...
                    diag = diag
                        .with_recommendation(format!(
                            "Consider TCP MSS clamping to {} bytes",
                            max_successful - 40
                        ))
            ...
            result.add_metadata(
                "verdict",
                "No artificial TCP segment size restrictions detected",
            );
        }
