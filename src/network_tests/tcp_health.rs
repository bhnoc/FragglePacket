...
use crate::framework::result::{Diagnosis, DiagnosisSeverity, TestResult, TestStatus};
...
impl NetworkTest for TcpHealthTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        result.add_metadata(
            "cli_command",
            format!(
                "curl -so /dev/null -w 'TCP connect: %{{time_connect}}s' https://{}",
                target
            ),
        );
        ...
        result.add_metadata(
            "cli_note",
            "Tests TCP handshake timing and connection health",
        );
        ...
        // Overall status
        if result.diagnoses.iter().any(|d| {
            matches!(
                d.severity,
                DiagnosisSeverity::Error | DiagnosisSeverity::Critical
            )
        }) {
...
fn test_window_size(target: &str, port: u16) -> Result<usize, Box<dyn Error>> {
    ...
    #[cfg(target_os = "linux")]
    {
        use libc::{c_void, getsockopt, socklen_t, SOL_SOCKET, SO_RCVBUF};
