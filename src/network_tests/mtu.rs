...
impl NetworkTest for IcmpMtuTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        result.add_metadata(
            "cli_note",
            "Binary search from 576-1500 bytes with DF bit set",
        );
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            result.add_metadata(
                "reason",
                "ICMP MTU discovery uses Linux-specific ping flags (-M do)",
            );
            result.add_metadata("alternative", "Use TCP MSS Discovery instead");
            result.add_metadata(
                "macos_alt",
                "Use: ping -D -s 1472 (but no DF flag on macOS)",
            );
...
impl NetworkTest for TcpMtuTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        result.add_metadata(
            "cli_command",
            format!("curl -so /dev/null -w 'TCP connected' https://{}", target),
        );
        result.add_metadata(
            "cli_tcpdump",
            format!(
                "tcpdump -i any -c 5 'tcp[13] == 2' and host {} | grep mss",
                target
            ),
        );
