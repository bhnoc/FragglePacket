...
impl NetworkTest for DnsTest {
    ...
    fn run(&self, target: &str) -> Result<TestResult, Box<dyn Error>> {
        ...
        result.add_metadata(
            "cli_multi_dns",
            format!(
                "dig @1.1.1.1 {} +short; dig @8.8.8.8 {} +short",
                target, target
            ),
        );
        ...
        if !server_results.is_empty() {
            // Find fastest server
            if let Some((fastest_server, fastest_time)) = server_results
                .iter()
                .min_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
            {
                ...
                if let Some((slowest_server, slowest_time)) = server_results
                    .iter()
                    ...
                    if diff > 500.0 {
                        result.add_diagnosis(
                            Diagnosis::new(
                                ...
                                format!(
                                    "DNS server performance varies by {:.0}ms ({} vs {})",
                                    diff, fastest_server, slowest_server
                                ),
                            )
