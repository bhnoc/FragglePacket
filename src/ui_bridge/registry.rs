...
pub const COMMANDS: &[Cmd] = &[
    ...
    Cmd { name: "diagnose", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Full diagnostic against a hostname (DNS, TCP, HTTP, ICMP comparison)" },
    Cmd { name: "dns-secure", bucket: Bucket::DnsAddressing, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "DoH/DoT vs plain DNS comparison" },
    ...
    Cmd { name: "https", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Test HTTPS connectivity with stage-by-stage analysis (MTU blackhole detection)" },
    ...
    Cmd { name: "multi", bucket: Bucket::PathMtu, required_inputs: &["TARGETS"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Test multiple targets and compare path MTUs" },
    ...
    Cmd { name: "printer-raw", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Raw JetDirect port 9100 PJL + bulk size sweep" },
    ...
    Cmd { name: "quic", bucket: Bucket::PathMtu, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "QUIC/UDP PMTUD probe" },
    Cmd { name: "quick", bucket: Bucket::PathMtu, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Quick ICMP-only MTU test" },
    ...
    Cmd { name: "report", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Render a unified README_FIRST-style diagnosis of a target" },
    ...
    Cmd { name: "scenario", bucket: Bucket::Tools, required_inputs: &["FILE"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Run a declarative scenario from a file or stdin" },
    ...
    Cmd { name: "ssh-path", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "SSH banner + optional authenticated echo data-path test" },
    ...
    Cmd { name: "tcp", bucket: Bucket::PathMtu, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "TCP-based MTU discovery (no ICMP required)" },
    Cmd { name: "tcp-options", bucket: Bucket::PathMtu, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Query actual negotiated TCP MSS and detect middlebox rewriting" },
    ...
    Cmd { name: "test", bucket: Bucket::Tools, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Run test framework tests (DNS, HTTPS, TCP, RTT, Loss)" },
    ...
    Cmd { name: "upload-sweep", bucket: Bucket::PathMtu, required_inputs: &["TARGET"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "HTTP(S) upload size sweep (detects data-stall blackholes)" },
    Cmd { name: "vpn", bucket: Bucket::PathMtu, required_inputs: &["VPN_TYPE"], emits_json: true, platform: Platform::AnyPlatform, needs_privilege: false, gaps: None, summary: "Calculate safe MTU for VPN/SASE/Zero-Trust usage" },
...
mod tests {
    ...
    fn json_flag_is_added_only_where_supported() {
        ...
        // `serve` runs a long-lived Prometheus endpoint: its output IS the
        // /metrics surface, so there is no result document to serialize. Chosen
        // over a command that merely lacks JSON today, since this one never will.
        let without = find("serve").expect("serve is registered");
        assert!(!without.emits_json, "serve has no --json mode");
