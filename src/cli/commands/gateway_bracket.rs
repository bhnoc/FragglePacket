...
fn run_real(
    ...
    let download_budget =
        LoadBudget::maintenance(args.rate_mbps, args.phase_duration_secs.max(1), 1);
    ...
    let simultaneous_budget =
        LoadBudget::maintenance(args.rate_mbps * 2.0, args.phase_duration_secs.max(1), 2);
...
fn print_human(report: &GatewayBracketReport) {
    ...
    println!(
        "{}",
        "== Gateway Latency-Under-Load Bracket ==".cyan().bold()
    );
    println!(
        "  interface: {}{}",
    ...
    if report.data_source == "synthetic" {
        println!(
            "  {} this report is SYNTHETIC (--inject-synthetic), not a real probed run",
            "⚠".yellow()
        );
    }
    println!();
    ...
    for phase in &report.phases {
        ...
        match phase.icmp_state {
            IcmpState::Responding => println!(
                "    icmp: {}/{} received",
                phase.icmp_received, phase.icmp_sent
            ),
        ...
        println!(
            "    rtt avg={} max={}",
            fmt_ms(phase.avg_rtt_ms),
            fmt_ms(phase.max_rtt_ms)
        );
