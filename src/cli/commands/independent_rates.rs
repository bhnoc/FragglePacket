...
fn synthetic_sweep(direction: Direction) -> DirectionSweep {
    ...
    let points = match direction {
        Direction::Upload => vec![
            RatePoint {
                target_mbps: 250.0,
                achieved_mbps: Some(249.9),
                loss_percent: Some(0.02),
                usable: true,
            },
            RatePoint {
                target_mbps: 300.0,
                achieved_mbps: Some(283.0),
                loss_percent: Some(5.6),
                usable: true,
            },
            RatePoint {
                target_mbps: 350.0,
                achieved_mbps: Some(302.0),
                loss_percent: Some(13.6),
                usable: true,
            },
        ],
        Direction::Download => vec![
            RatePoint {
                target_mbps: 250.0,
                achieved_mbps: Some(249.8),
                loss_percent: Some(0.076),
                usable: true,
            },
            RatePoint {
                target_mbps: 300.0,
                achieved_mbps: Some(242.0),
                loss_percent: Some(19.3),
                usable: true,
            },
            RatePoint {
                target_mbps: 350.0,
                achieved_mbps: Some(246.0),
                loss_percent: Some(29.7),
                usable: true,
            },
        ],
    };
...
pub fn run(args: &IndependentRatesArgs) {
    ...
    let (upload_sweep, download_sweep, example_merged) = if args.inject_synthetic {
        (
            synthetic_sweep(Direction::Upload),
            synthetic_sweep(Direction::Download),
            None,
        )
    } else {
    ...
    let report = IndependentRatesReport {
        ...
        data_source: if args.inject_synthetic {
            "synthetic"
        } else {
            "live"
        },
...
fn run_real(
    ...
    for (i, &rate) in rates.iter().enumerate() {
        ...
        let upload_result = upload_handle.join().unwrap_or_else(|_| {
            Err(IperfParseError::InvalidJson(
                "upload thread panicked".to_string(),
            ))
        });
        ...
        if i == 0 {
            example_merged = Some(merge_timeline(
                SessionWindow {
                    ...
                    start_secs: upload_start,
                    end_secs: upload_start + elapsed_guess,
                },
                SessionWindow {
                    direction: Direction::Download,
                    port: args.download_port,
                    start_secs: download_start,
                    end_secs: download_start + elapsed_guess,
                },
            ));
        }
...
fn fmt_first_lossy(f: &FirstLossyRate) -> String {
    match f {
        ...
        FirstLossyRate::AllTestedRatesLossy => {
            "every tested rate was already lossy -- no clean baseline to report a threshold against"
                .to_string()
        }
        FirstLossyRate::InsufficientData => {
            "insufficient data (fewer than two usable rate points)".to_string()
        }
    }
}
...
fn print_human(report: &IndependentRatesReport) {
    ...
    println!(
        "{}",
        "== Independent Upload/Download Rate Sweep ==".cyan().bold()
    );
    println!(
        "  server: {} interface: {} source={}",
        report.server, report.interface, report.data_source
    );
    println!(
        "  upload_port={} download_port={}",
        report.upload_port, report.download_port
    );
    println!();
    println!("  {}", "Upload:".bold());
    for p in &report.upload_sweep.points {
        println!(
            ...
            p.target_mbps,
            fmt_mbps(p.achieved_mbps),
            fmt_pct(p.loss_percent),
            p.usable
        );
    }
