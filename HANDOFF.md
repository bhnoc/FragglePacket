...
5. **Never commit a real capture.** `harness/fixtures/pcap/*` are synthetic,
   generated with a local `re-cap-inator`; see `harness/fixtures/README.md`.
   `harness/checks/001-fixture-privacy.sh` decodes every binary capture and
   fails on a globally-unique MAC, because the text rules there use `grep -I`
   and cannot see inside a pcap. When reading a large capture locally, use
   `tshark -c N` and carve with `editcap -s 128`; `tshark -s` does not truncate
   on file reads, only on live capture.
