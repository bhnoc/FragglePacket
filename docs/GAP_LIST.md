...
| GAP-073 | P0 | Aggregate verdicts are computed only over probes that answered, so unreachable targets raise the pass rate | `kitchen-sink` collects "all successful MTU measurements" and then derives `pct_ok = at_1500 / mtu_values.len()`. A target that never answered contributes no entry, so it leaves the denominator entirely. With 18 of 20 targets unreachable and the 2 that answered at 1500, the run prints `PASS - No MTU changes needed` at "100% of tests at 1500 MTU". A near-total connectivity failure is therefore reported as a clean bill of health, in the command a new user is most likely to run first. | Separate attempted from measured everywhere a rate or percentage is derived from a set of probes. Report both counts, express the ratio against attempts, and refuse or explicitly downgrade the verdict when measured coverage falls below a declared threshold. A probe that did not answer is missing evidence, never a silent pass; an all-skipped set must never aggregate to success. |
| GAP-074 | P1 | Contradiction between two evidence sources is resolved silently instead of being reported | Commands that hold both actively measured and operator-supplied values for the same property pick one and continue. `wired_edge.rs` and `ap_compat_matrix.rs` ingest operator telemetry as vendor-neutral fields, and `probe`/`mss-evidence` measure comparable properties directly, but nothing compares the two and nothing reports when they disagree. A switch reporting a 1 Gb link while the client measures 5 Gb, or operator-declared MTU 9000 against a probed 1500, is the single most diagnostic observation available and is currently discarded. | Add a reusable corroboration verdict distinguishing corroborated, uncorroborated (only one source present), and contradicted. When two independent sources disagree beyond a per-property tolerance, emit the disagreement as its own finding carrying both values and both provenances, and withhold any derived figure that depends on the contested property rather than silently preferring one source. |
| GAP-075 | P2 | Observations carry no validity horizon, so a figure derived minutes later is reported as current | A long sweep samples radio, interface, and route state once at the start and derives figures from it for the rest of the run. GAP-035 guards phase boundaries against association changes, but nothing invalidates a figure whose *input* aged out mid-run: a roam, DHCP renewal, or link-speed renegotiation at t=30s leaves every later figure derived from the t=0 snapshot presented as though freshly measured. | Stamp each observation with the time it was taken and a per-property validity horizon. Before emitting a derived figure, check that every input is still inside its horizon; when one is not, mark the input stale and withhold or requalify the derived figure rather than reporting it as current. Staleness must read as "not known now", never as the last known value. |

## Resolved during this investigation

| Item | Resolution |
| --- | --- |
| GAP-073, GAP-074, GAP-075 (coverage denominator, source disagreement, observation freshness) | **Closed 2026-08-04**, all three found by gap-listing this project against [nocproject/noc](https://github.com/nocproject/noc). NOC is a continuous carrier OSS/NMS and its architecture is not applicable here, but three of its evidence-discipline rules were: `get_check_status()` returning `failed` when every check was skipped (vacuous truth), bilateral link confirmation refusing to assert a link one side denies, and `CheckStatus.expired` dropping a stale answer into unknown rather than serving it as current. Closed by `network_tests/coverage.rs` (gate 075), `network_tests/corroboration.rs` (gate 076), and `network_tests/freshness.rs` (gate 077). Wiring the freshness horizon into `phy_normalized` surfaced three further pre-existing defects: a stratum mean dividing a filtered sum by an unfiltered count, a zero PHY capacity yielding `NaN`, and cohort attribution comparing against an absent mean. Notably NOC's own counter-wrap handling is *weaker* than ours -- it infers wrap width from the previous value, where `counter_liveness.rs` disambiguates against a known stimulus -- and it has no counter-liveness validation at all, which is our GAP-043. |
