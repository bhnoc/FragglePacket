...
| GAP-067 | P1 | No secure CV-CUE/Arista telemetry connector | The investigation required manual 1Password retrieval, Launchpad tenant discovery, session-cookie handling, response-shape inspection, client/AP joins, identifier hashing, and live extraction of retry, contention, interference, RF utilization, power, firmware, and switch-port linkage. Client-only results missed this infrastructure evidence. | Add a read-only infrastructure connector that retrieves credentials from a secret provider without logging them, discovers the assigned tenant, uses temporary cookies, allowlists fields, hashes AP/chassis identities, handles pagination and null/schema variants, and joins controller snapshots to named test phases. Refuse mutation methods and delete raw responses on exit by default. |
| GAP-068 | P1 | No CV-CUE historical performance and event import | CV-CUE retains historical client performance and related AP events, but the basic `/clients` time-filter query returned only one current association record and could not reconstruct retry, utilization, crashes, config changes, or roaming during the earlier incident. | Import an operator-exported or authorized API time series for client journey, RF KPIs, AP health, related AP events, crashes/reboots, configuration changes, and tunnel state. Normalize timestamps, distinguish current snapshots from historical samples, and align events with signed FragglePacket phase boundaries. |
...
| GAP-071 | P1 | No effective WLAN configuration snapshot, cross-AP diff, or visibility-gap report | Manual API joins were required to resolve each client's location policy, actual AP template, active radio, and matching SSID profile. PC13 and PV10 had identical relevant radio/QoS settings despite different PoE/uplink states. The accessible schema exposed WMM, QoS mappings, shaping, aggregation, OFDMA/MU-MIMO, spatial reuse, steering, and rate controls, but not the scheduler algorithm, airtime-fairness selection, per-AC queue depths, or TXOP limits. The separate `wirelessProtocol=BE` field also remained present after `wirelessMode` changed to A/N/AC, so treating it as the enabled standard would produce a false conclusion. | Add a privacy-safe read-only configuration collector that resolves location defaults, actual device templates, overrides, active radios, SSID profiles, and AP power/uplink state; records schema/version and absent-versus-zero values; distinguishes platform capability from enabled standards; diffs affected and control APs; redacts security and topology identifiers; and explicitly reports scheduler/airtime/queue fields that the role or schema cannot expose. |
