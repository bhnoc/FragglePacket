...
| GAP-058 | P1 | No wired edge, AP uplink, LLDP, and PoE health bundle | C-460 behavior can depend on PoE allocation, negotiated Ethernet speed, redundant uplinks, switch errors, VLAN configuration, or reduced-functionality mode. Client tests cannot see those conditions. | Ingest switch/AP telemetry for LLDP identity, PoE class/budget/draw, reduced-power state, link speed/duplex, LACP member state, VLAN/native-tag consistency, CRC/errors/discards, pause frames, queue drops, and link flaps. Bracket counters around the client test timeline. |
